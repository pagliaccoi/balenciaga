...
function initCategoryScrollbar() {
            ...
            function updateScrollbar() {
                ...
                const thumbOffset =
                    maxThumbOffset * (list.scrollLeft / maxScroll);
            ...
            scrollbar.addEventListener('pointermove', (event) => {
                ...
                if (dragState.isThumb) {
                    ...
                    list.scrollLeft =
                        dragState.startScroll + scrollDelta;
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // 2. 아코디언 메뉴 토글 기능
            const accordionHeaders = menuLayer.querySelectorAll('.filter-accordion-header');
            ...
            // 3. 원본 상품 데이터 캐싱
            const originalItems = Array.from(productListEl.querySelectorAll('li')).map(li => {
                const isBanner = li.classList.contains('women-img1') || li.classList.contains('women-img2') || !li.querySelector('.product-price');
            ...
            function getFilteredList() {
                ...
                if (currentPrice === 'under2m') {
                    workingList = workingList.filter(item => item.isBanner || item.price < 2000000);
                } else if (currentPrice === 'over4m') {
                    workingList = workingList.filter(item => item.isBanner || item.price > 4000000);
                }

                if (currentSort === 'lowPrice') {
                    workingList.sort((a, b) => (a.isBanner ? 1 : b.isBanner ? -1 : a.price - b.price));
                } else if (currentSort === 'highPrice') {
                    workingList.sort((a, b) => (a.isBanner ? 1 : b.isBanner ? -1 : b.price - a.price));
                } else if (currentSort === 'name') {
                    workingList.sort((a, b) => (a.isBanner ? 1 : b.isBanner ? -1 : a.title.localeCompare(b.title, 'ko')));
                } else if (currentSort === 'default') {
                    workingList.sort((a, b) => originalItems.indexOf(a) - originalItems.indexOf(b));
                }
            ...
            if (resetBtn) {
                resetBtn.addEventListener('click', () => {
                    ...
                    menuLayer.querySelectorAll('ul button').forEach(b => b.classList.remove('active'));
